describe('display dicom image', () => {
  it('shows dicom image', async () => {
    ...
    await cy.task('orthancResponds')
    // const [serieId] = await cy.task('findOrthancSerieByDate').promisify()
    // console.log(serieId)
    // cy.log(serieId)
    // const seriesInstances = await cy.task('getOrthancSeriesInstances', serieId).promisify()
    // console.log(seriesInstances)
    // cy.log(`seriesInstances.length ${seriesInstances.length}`)
